...
## Maria STR
STR_payout = data.frame(yearmonth = c(paste0('2025-0',1:9),paste0('2025-',10:12)),
...
daily = cleanings_guesty_residential %>% 
  ...
  reframe(units = n(),
          ...
          CleaningFee.Residential = sum(residential.fee, na.rm = T),
...
daily = merge(daily, payCleaner %>% 
  ...
  mutate(yearmonth= substr(Cleaning.Date, 1, 7),
         ...
         CleaningFee_Residential_cum = unlist(tapply(CleaningFee.Residential, yearmonth, cumsum)))
...
Daily cleaning based on dispatch card 

```{r daily-summary-dispatch}
daily_dispatch = data %>% 
        filter(!duplicated(paste(Cleaning.Date,Listing))) %>%
        group_by(Cleaning.Date) %>%
        reframe(units = n(),
                units_STR = sum(!Type %in% "Residential"),
                units_Residential = sum(Type %in% "Residential"),
                CleaningIncome = sum(CleaningIncome, na.rm = T),
                CleaningIncome.STR = sum(CleaningIncome[!Type %in% "Residential"], na.rm = T),
          CleaningIncome.Residential = sum(CleaningIncome[Type %in% "Residential"], na.rm = T)) 

daily_dispatch = merge(daily_dispatch ,payCleaner %>% 
                group_by(Week, Day) %>% 
                reframe(CleaningCost = sum(Rate,na.rm=T)),
              by.x = 'Cleaning.Date', by.y = 'Day', suffix = c("", ".pay"))

summary_stats_dispatch = daily_dispatch %>% 
    reframe(
    Days = n(),
    Total_Units = sum(units, na.rm = TRUE),
    STR_Units = sum(units_STR, na.rm = TRUE),
    Residential_Units = sum(units_residential, na.rm = TRUE),
    CleaningIncome = sum(CleaningIncome, na.rm = TRUE),
    CleaningCost = sum(CleaningCost, na.rm = TRUE),
    delta_income_cost = round(CleaningIncome -CleaningCost,2),
...
monthly_dispatch = daily_dispatch %>% 
  mutate(yearmonth = substr(Cleaning.Date,1,7)) %>%
  group_by(yearmonth) %>% 
  reframe(units = n(),
                units_STR = sum(units_STR),
                units_Residential = sum(units_Residential),
                CleaningIncome = sum(CleaningIncome, na.rm = T),
                CleaningIncome.STR = sum(CleaningIncome.STR),
                CleaningIncome.Residential = sum(CleaningIncome.Residential),
