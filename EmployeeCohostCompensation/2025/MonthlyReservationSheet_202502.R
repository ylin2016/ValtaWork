...
length(unique(reservations$Listing)) #65
...
STR_table = reservations %>% filter(Status %in% 'confirmed') %>%
  ...
  mutate(d202502 = ifelse(STRs>0,"STR",""))
STR_Table = merge(org_table[,c("Listing","rank")],STR_table[,c("Property",'d202502')],
