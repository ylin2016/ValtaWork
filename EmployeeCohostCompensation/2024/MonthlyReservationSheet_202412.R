...
for(k in sort(unique(reservations$Cohost)))
 {
  ...
  { 
    ...
    if(k %in% c("Alice","Crystal","Yumiko","Eddie","Shaya","Felecia","Paul","Shaya","Anna",
...
STR_table = reservations %>% filter(Status %in% 'confirmed') %>%
  ...
  mutate(d202412 = ifelse(STRs>0,"STR",""))
STR_Table = merge(org_table[,c("Listing","rank")],STR_table[,c("Property",'d202412')],
