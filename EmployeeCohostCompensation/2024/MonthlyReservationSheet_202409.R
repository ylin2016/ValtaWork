...
cohost = merge(cohost,employee,by.x='Cohost',by.y ='Nick.name',all.x=T) %>% 
  arrange(Listing)
#View(cohost)
reviews = read.xlsx('./Working/Data/2024/guesty_reviews.xlsx')
##-----------------------------------------------------------------------------------
######## booking records ########
...
trips[trips$Cohost %in% 'Niya' & trips$CheckIn %in% '2024-09-30',
      c("Status",'MonthlyTrashCollection','CohostPayOut')] = c("MonthlyTrashCollection",50,50)

trip.tmp = trips[trips$Cohost %in% 'Niya' & trips$CheckIn %in% '2024-09-30',]
trips.trashes=NULL
for(i in c(1:5,7,8))
  {
    tmp = trip.tmp
    tmp$CheckIn = paste0('2024-0',i,'-29')
    trips.trashes = rbind(trips.trashes,tmp)
}
...
reservations = merge(reservations,
                     reviews %>% filter(!is.na(Reservation) & !is.na(Overall)) %>% select(Reservation,Overall),
...
length(unique(reservations$Listing)) #61

file_loc = "./Cohost's reservation sheets/ReservationSheet_"
for(k in sort(unique(reservations$Cohost))[-c(13)])
 {
  print(k)
  temp <-reservations %>% 
    filter(Cohost %in% k) %>% 
  ...
  { 
    ...
    if(k %in% c("Yumiko","Eddie","Shaya","Felecia","Paul","Shaya","Anna",
...
sum_property = reservations %>% 
  group_by(Cohost,Property) %>% 
    reframe(Month = substr(enddate,1,7),
            ...
            Nreviews = sum(!is.na(Overall)),
            SumRating = sum(Overall,na.rm=T),
            NamesRate = paste(names(table(Overall)),collapse = ';'),
            Rates = paste(unlist(table(Overall)),collapse=';')) %>%
...
sum_cohost = sum_property %>% group_by(Cohost) %>% 
        reframe(Month = substr(enddate,1,7),
                ...
                AverageRating = round(ifelse(sum(Nreviews)==0,NA,sum(SumRating)/sum(Nreviews)),2),
                Rates = sapply(c(Rate1,Rate2,Rate3,Rate4,Rate5),function(x)
                          ifelse(sum(x)>0,sum(x),NA))) %>%
  relocate(Month,.before=Cohost)

sum_rates = 
  reservations %>% group_by(Cohost) %>% 
  reframe(Nreviews = sum(!is.na(Overall)),
          AverageRating = ifelse(sum(!is.na(Overall))==0,NA,round(mean(Overall,na.rm=T),2)),
          Rates = paste(paste0(names(table(Overall)),":(",table(Overall),")"),collapse = ';')) %>%
  mutate(Rate_N = ifelse(is.na(AverageRating),NA,Rates))
...
cleaner = employee = read.xlsx('./Working/Data/Property_Cohost.xlsx',sheet='Cleaning')
...
cleansheet = cleansheet %>% 
  ...
  #mutate(Cleaner.lead = ifelse(Listing %in% 'Seattle 1623',"Maria",Cleaner.lead))
...
cleaning.output = list(CleaningSheet202408 =cleansheet,
