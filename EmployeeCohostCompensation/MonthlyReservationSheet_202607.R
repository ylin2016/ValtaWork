...
# 3/1/2026: add bimonthly inspection records for remote, $80/property
##======================================================================
...
## 3. refund carryover the months:
## use revenue recording file to identify with refund>0 ones

confirmed = confirmed %>% filter(!(grepl('owner|Owner',Source)| Earnings %in% 0)) #319
dim(confirmed)
cancelled = cancelled_input(fileloc,filemonth) #159
dim(cancelled)
...
dups = duplicated_reservations_lastmonth(confirmed,prior1="2026-06-01")
#combResvId = c(combResvId,dups[c(1,2)])
#only 3 need to combine
confirmed = update_duplicated_confirmed(confirmed,combResvId[5])

confirmed = rbind(confirmed %>% 
  filter(!Confirmation.Code %in% c('HMQPJB8E3Q','HMPNXF39ZM')),
      update_duplicated_confirmed(confirmed %>% 
          filter(Confirmation.Code %in% c('HMQPJB8E3Q','HMPNXF39ZM')),combResvId[2]))
dim(confirmed) #458
...
#inspections = inspections %>% filter(!duplicated(Property))
...
trips.all = combine_trips(trips)#,inspections)
View(trips.all) #20
##-----------------------------------------------------------------------------------
reservations = combine_reservations(confirmed,cancelled)
dim(reservations) #617
...
length(unique(reservations$Listing)) #89
...
sum(output$Cohost %>% 
      filter(Month %in% filemonth & !Cohost %in% 'Shaya') %>% 
...
Counts = merge(cleaning.output$CleaningPerProperty %>% 
               ...
               cleaner[,c("Listing","Cleaner.pay")],by="Listing",all.x=T) %>%
