...
confirmed = read.csv('./Working/Data/2023/Guesty_booking_confirmed_Nov2023.csv')
cancelled = read.csv('./Working/Data/2023/Guesty_booking_cancelled_Nov2023.csv')
...
## add trip data
trips  = read.xlsx('./Working/Data/2023/TripLog_Nov2023.xlsx')
inspections = read.xlsx('./Working/Data/2023/Bi_monthly_Inspection_Nov2023.xlsx')
...
for(k in sort(unique(reservations$Cohost))[-(1:4)])
 {
  temp <-reservations %>% 
    filter(Cohost %in% k) %>% 
  ...
  old = old %>% filter(Month %in% c('2023-08','2023-09','2023-10')) %>% 
