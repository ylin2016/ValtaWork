...
ownerstay = confirmed %>% 
  filter((grepl('owner|Owner',Source)&!grepl("Yacinde",Listing))| 
           (Earnings %in% 0 & !grepl("Yacinde",Listing))) 

all_sheets[[1]] = rbind.fill(all_sheets[[1]] %>% 
                        filter(CheckOut<startdate), ownerstay) %>% 
...
all_sheets[[2]] = rbind.fill(all_sheets[[2]]%>% 
                               filter(CheckOut<startdate),nocleaning)  %>%
                mutate(CheckIn = as.Date(CheckIn,origin= '1899-12-30'),
                CheckOut = as.Date(CheckOut,origin= '1899-12-30'))
...
dim(confirmed) #587
cancelled = cancelled_input(fileloc,filemonth) #178
dim(cancelled)
...
dups = duplicated_reservations_lastmonth(confirmed,prior1="2026-07-01")
...
cleaner_thisyear = c("Owner(Sophia)","Brittany",'Angelina',"Andrea")
