...
confirmed = format_reservation(confirmed,startdate,enddate) #312
...
confirmed0 = read.csv(paste0(fileloc,'Guesty_booking_confirmed_2025-05.csv'))
...
confirmed[paste(confirmed$Listing,confirmed$GuestName) %in% combResvId,] %>%
...
cancelled$LISTING.S.NICKNAME[grep("Monthly",cancelled$LISTING.S.NICKNAME)]="Seattle 9750"
...
table(inspections$BEDROOMS,exclude=NULL)
...
STR_table = reservations %>% filter(Status %in% 'confirmed') %>%
  ...
  mutate(d202506 = ifelse(STRs>0,"STR",""))
