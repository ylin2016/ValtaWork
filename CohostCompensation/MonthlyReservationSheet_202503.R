...
load("./Working/Backup_adjustment_202503.Rdata")
reservations = rbind(reservations,res_adj)
...
sum_cohost = rbind(sum_cohost,sum_cohost_adj) %>% arrange(Cohost,Month)
sum_property = rbind(sum_property,sum_property_adj) %>% 
  arrange(Cohost,Property,Month)
...
STR_table = reservations %>% filter(Status %in% 'confirmed') %>%
  ...
  mutate(d202503 = ifelse(STRs>0,"STR",""))
STR_Table = merge(org_table[,c("Listing","rank")],STR_table[,c("Property",'d202503')],
