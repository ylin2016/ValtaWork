...
length(unique(reservations$Listing)) #60
...
sum_cohost_all = rbind.fill(sum_cohost,sum_coh %>% filter(!Month %in% substr(enddate,1,7))) %>% 
  relocate(Bimonthly,TrashMonthly,.before = Earnings)
output = list(Cohost=sum_cohost_all,Property = sum_property_all)
View(output$Cohost)
output = list(Cohost=sum_coh,Property = sum_property_all)
...
cleaning.output = list(CleaningSheet202405 =cleansheet,
