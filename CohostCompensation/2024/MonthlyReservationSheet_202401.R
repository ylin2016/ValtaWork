...
## make changes 
idx = reservations$Listing %in% c('Seattle 1424C','Seattle 1430B') & reservations$CheckOut<='2024-01-02'
reservations[idx,colnames(cohost)[-c(2:11)]] = cohost[cohost$Cohost %in% 'Yi',colnames(cohost)[-c(2:11)]][1:sum(idx),]
idx = reservations$Listing %in% c('Poulsbo 13976','Poulsbo 3956','Poulsbo 563','Seatac 12834',
          'Seatac 12834 Lower','Seatac 12834 Upper') & reservations$CheckOut>='2024-01-25'
reservations[idx,colnames(cohost)[-c(2:11)]] = cohost[cohost$Cohost %in% 'Rachel',colnames(cohost)[-c(2:11)]][1,]

# reservations = rbind.fill(reservations,reservations_backup,trips.all)
reservations = rbind.fill(reservations,trips.all)
length(unique(reservations$Listing)) #47
