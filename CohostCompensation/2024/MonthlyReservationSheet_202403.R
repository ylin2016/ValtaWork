...
length(unique(reservations$Listing)) #58
...
for(k in sort(unique(reservations$Cohost))[-(1:11)])
 {
  print(k)
  temp <-reservations %>% 
    filter(Cohost %in% k) %>% 
...
cleaning.output = list(CleaningSheet =cleansheet,CleaningPerProperty=summary_cleaning,PayOut = PayOut)
write.xlsx(cleaning.output,"./Cohost's reservation sheets/CleaningSheet_PayOut.xlsx",
           firstActiveRow = 2,withFilter = T)
