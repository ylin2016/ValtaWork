...
# combResvId = paste(confirmed$Listing,confirmed$GuestName)[dup][-1] #2 of 3 needed
# combResv = confirmed %>% 
...
## 8/8/2024	Xu will be the cohost for 8415
reservations[reservations$Listing %in% 'Seattle 8415' & reservations$CheckIn<='2024-08-07',
             c('Cohost',"Regular.rate")] = c('Sasha',30)
reservations$Regular.rate = as.integer(reservations$Regular.rate)
...
for(k in sort(unique(reservations$Cohost)))
 {
  ...
  { 
    ...
    if(k %in% c("Felecia","Paul","Shaya","Anna","Thao","Feifei","JingZhou","Lulu","Sasha","YongChao")) ## new this year
    { 
      write.xlsx(list("2024"=temp),paste0(file_loc,CohostName,'.xlsx'),
                 na.strings=c(NA,""),firstActiveRow = 2,withFilter = T)
    }else{
...
cleansheet = merge(confirmed[,c("Listing",'CheckIn','CheckOut','Guests','Nights')],
                   cleaner[,c("Listing","Listing","Cleaner.lead",'Cleaning.fee')],
                   by="Listing",all.x=T)
