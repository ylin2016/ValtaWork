...
#View(cohost)
######## booking records ########
confirmed = confirmed_input(fileloc,filemonth) #338 ## has Redmond16021
...
dups = duplicated_reservations_lastmonth(confirmed,prior1="2025-08-01")
#combResvId = c(combResvId,dups[2])
# only 1 need to combine
...
trips = trips_input(fileloc,filemonth,employee,LRTs=c(1,3,4,5,7,8,10))
...
length(unique(reservations$Listing)) #69
...
cleaner_sheets(cleaning.output$CleaningSheet %>% filter(!Cleaner.lead %in% 'Maria'))
...
Counts = rbind.fill(cleaning.output$CleaningPerProperty %>% 
                      filter(Cleaner.lead %in% "Maria"),Counts)
...
completed = rbind(currentmonth %>% filter(!is.na(Completion.Date)) %>% 
        ...
        lastmonth %>% filter(!is.na(Completion.Date)) %>% 
        select(Property,Completion.Date)) %>%
       mutate(Completion.Date = as.Date(as.integer(Completion.Date),
                       origin= '1899-12-30')) %>%
      arrange(Property) %>% unique()
...
output = list(Due.2510=nextmonth,
              Due.2509=currentmonth,
              Due.2508=lastmonth,
              Schedules = bimonth)
write.xlsx(output, paste0(filesave,"Property_due_bimonthly_inspection.xlsx"),
           na.strings=c(NA,""),firstActiveRow = 2,withFilter = T)
