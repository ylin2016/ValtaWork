...
sum(dup<-duplicated(paste(confirmed$Listing,confirmed$GuestName))) #0
...
trips.trashes = data.frame(Month =  substr(enddate,1,7),
                           Property= 'Seattle 1632',
                           Cohost='Niya',
                           CheckIn = as.Date("2025-01-31"),
                           Status = "MonthlyTrashCollection",
                           CohostPayOut = 50)
## Eddies's Long term lease fee
...
length(unique(reservations$Listing)) #64

file_loc = "./Cohost's reservation sheets/ReservationSheet_"

# function(temp,file_loc,CohostName,Year="2025"){
#   wb <- loadWorkbook(paste0(file_loc,CohostName,'.xlsx'))
#   addWorksheet(wb,Year)
#   writeData(wb,sheet=Year,x=temp,na.string="",withFilter = T)
#   saveWorkbook(wb,paste0(file_loc,CohostName,'.xlsx'),overwrite = TRUE)
# }
for(k in sort(unique(reservations$Cohost)))
 {
  ...
  old2024 = read.xlsx(paste0(file_loc,CohostName,'.xlsx'),sheet='2024')
  if(class(old2023)=="try-error"){
    write.xlsx(list("2025"=temp,"2024"=old2024),paste0(file_loc,CohostName,'.xlsx'),
               na.strings=c(NA,""),firstActiveRow = 2,withFilter = T)
  }else{
   write.xlsx(list("2025"=temp,"2024"=old2024,"2023"=old2023),paste0(file_loc,CohostName,'.xlsx'),
             na.strings=c(NA,""),firstActiveRow = 2,withFilter = T)
  }
 }
...
#           na.strings=c(NA,""),firstActiveRow = 2,withFilter = T)
#   }else{
#     old = read.xlsx(paste0(file_loc,CohostName,'.xlsx'))#,sheet='2025')
...
sum_property_all = sum_property %>%
  relocate(Bimonthly,TrashMonthly,.before = Earnings)
sum_cohost_all = sum_cohost %>%
  relocate(Bimonthly,TrashMonthly,.before = Earnings)
...
STR_table = reservations %>% filter(Status %in% 'confirmed') %>%
  ...
  mutate(d202501 = ifelse(STRs>0,"STR",""))
STR_Table = merge(org_table[,c("Listing","rank")],STR_table[,c("Property",'d202501')],
...
for(k in unique(cleansheet$Cleaner.lead))
{
  ...
  #temp = rbind.fill(temp,old)
  write.xlsx(list("2025"=temp,"2024"=old),paste0(cleaning_loc,k,'.xlsx'),
             na.strings=c(NA,""),firstActiveRow = 2,withFilter = T)
}