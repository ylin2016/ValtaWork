...
for(k in 1:nrow(files))
{
  ...
  tmp = sapply(listings,function(x,txt) {
    ...
    res = grepl(x,txt)|grepl(y,txt,fixed=T)
    },txt=files$file[k])
  if(sum(tmp)>0){
    if(sum(tmp)==1){
      files$property[k] = listings[tmp]
     }else{
      if("Maria" %in% listings[tmp]) files$property[k] = listings[tmp][1]
     }
  }
}
...
txts = c("Longbranch","Hoodsport","Keaau","Lilliwaup",
         ...
         paste0("OSBR ",c(1:10,12)), "OSBR 11","OSBR","1420","Beachwood 10 ")
chngs = c("Longbranch 6821","Hoodsport 26060","Keaau 15-1542","Lilliwaup 28610",
          "Beachwood",paste0("Beachwood ",1:10),
          paste0("Cottage ",c(1:10,12)),"Cottage 11 (tiny)","OSBR","Bellevue 1420",
          "Beachwood 10")
...
txts = c("Redmond16012","Cottage 3")
chngs = c("Redmond 16012","Cottage 3")
for(k in 1:length(txts))
{
...
idx = is.na(files$property) & files$property1 %in% listings
