...
def sync_qbo_expenses(conn, qbo, property_map: dict, account_rules: list[dict], start_date: str, end_date: str, exceptions_cb=None):
    ...
    # keyed by TxnDate in [start,end], so posting_date scoping matches exactly what we re-add.
    # build's downstream steps (osbr-rv, dup-deposit exclusion, central supplies) re-derive from
    # these rows and are themselves idempotent, so a clean re-sync + rebuild fully restores state.
    cur.execute("DELETE FROM ledger_lines WHERE source='qbo' AND posting_date>=? AND posting_date<=?",
                (start_date, end_date))
