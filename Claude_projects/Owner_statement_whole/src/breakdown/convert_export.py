...
def convert(input_file, output_file):
    ...
    # Filter: confirmed bookings OR canceled that RETAINED cash (TOTAL PAID > 0). Owner revenue
    # on a cancellation is only what the guest actually kept in; TOTAL PAID is ALREADY net of
    # refunds (do not subtract TOTAL REFUNDED again). A canceled booking with nothing retained is
    # not revenue and is excluded — matching payment_model, so Section 1 foots with the ledger.
    if "STATUS" in df.columns:
        confirmed = df["STATUS"].str.lower() == "confirmed"
        _retained = pd.to_numeric(df["TOTAL PAID"], errors="coerce").fillna(0)
        canceled_paid = (df["STATUS"].str.lower() == "canceled") & (_retained > 0)
        df = df[confirmed | canceled_paid]
        print(f"Filtered to {len(df)} rows (confirmed + canceled that retained cash)")
    ...
    def _net_revenue(idx, row):
        ...
        if is_cancelled:
            # Canceled bookings: net_revenue = retained cash = TOTAL PAID (already net of
            # refunds; the stay never happened, so only what the guest kept in is owner revenue).
