...
# ── Password gate (deploy only) ───────────────────────────────────────────────
# Real owner data on a public URL must be protected. Enforced ONLY when an
# `app_password` secret is configured (set it in Streamlit Cloud → Secrets, or in
# .streamlit/secrets.toml). With no secret set (e.g. local dev) the gate is off.
try:
